...
ADDRESSING_MODE::address_struct ADDRESSING_MODE::load_store_multiple(const arm_code_t &code) {
    if (
        (code.test(27) != true) || 
        (code.test(26) != false) ||
        (code.test(25) != false)
 ...
 */
ADDRESSING_MODE::address_struct ADDRESSING_MODE::ls_mul_inc_after(const arm_code_t &code) {
 ...
 *     Rn = Rn + (Number_Of_Set_Bits_In(register_list) * 4)
 */
ADDRESSING_MODE::address_struct ADDRESSING_MODE::ls_mul_inc_before(const arm_code_t &code) {
    const u16 list = shared::util::bit_range<u16>(code, 0, 15);
 ...
 *     Rn = Rn - (Number_Of_Set_Bits_In(register_list) * 4)
 */
ADDRESSING_MODE::address_struct ADDRESSING_MODE::ls_mul_dec_after(const arm_code_t &code) {
    const u16 list = shared::util::bit_range<u16>(code, 0, 15);
 ...
 *     Rn = Rn - (Number_Of_Set_Bits_In(register_list) * 4)
 */
ADDRESSING_MODE::address_struct ADDRESSING_MODE::ls_mul_dec_before(const arm_code_t &code) {
    const u16 list = shared::util::bit_range<u16>(code, 0, 15);
