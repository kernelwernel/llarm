...
[[nodiscard]] bool util::carry_add(const u64 &sum) {
    constexpr u64 max = ((1ULL << 32) - 1);
    return (sum > max);
}

[[nodiscard]] bool util::carry_add(const u32 sum1, const u32 sum2) {
    constexpr u64 max = ((1ULL << 32) - 1);
    return ((sum1 + sum2) > max);
}

// CHECK IF THIS WORKS
[[nodiscard]] bool util::carry_add(const u32 sum1, const u32 sum2, const u32 sum3) {
    constexpr u64 max = ((1ULL << 32) - 1);
    return ((sum1 + sum2 + sum3) > max);
}

// TODO: COMPLETE
[[nodiscard]] bool util::borrow_add(const u32 p1, const u32 p2) {
    return true; // TODO: COMPLETE THIS SHIT
}

[[nodiscard]] bool util::borrow_sub(const u32 p1, const u32 p2) {
    return (p1 < p2);
}

// CHECK IF THIS WORKS
[[nodiscard]] bool util::borrow_sub(const u32 p1, const u32 p2, const u32 p3) {
    return (((p1 - p2) - p3) < 0);
}

[[nodiscard]] bool util::overflow_add(const i32 a, const i32 b) {
    const i32 result = a + b;
    return (a > 0 && b > 0 && result < 0);
}

[[nodiscard]] bool util::overflow_add(const i32 a, const i32 b, const i32 c) {
...
[[nodiscard]] bool util::overflow_sub(const i32 a, const i32 b) {
...
// and the first operand and the result have different signs.
// TODO: double check if this works manually
[[nodiscard]] bool util::overflow_sub(const i32 a, const i32 b, const i32 c) {
...
[[nodiscard]] bool util::arithmetic_shift_right(u32 num, const u8 shift) {
