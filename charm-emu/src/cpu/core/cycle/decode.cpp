...
arm_decode_struct DECODE::arm_decode(const arm_code_t &raw_code) const {
    ...
    if (!condition_match(cond)) {
        tmp.id = charm::as::id::arm::NOP;
        tmp.code = arm_code_t(0);
    } else {
        tmp.id = charm::as::identify::arm(raw_code);
        tmp.code = raw_code;
    }
...
thumb_decode_struct DECODE::thumb_decode(const thumb_code_t &raw_code) const {
    ...
    tmp.id = charm::as::identify::thumb(raw_code);
    tmp.code = raw_code;
...
charm::as::id::jazelle DECODE::jazelle_decode(const u8 raw_code) const {
    return charm::as::identify::jazelle(raw_code);
}
