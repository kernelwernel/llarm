...
private:
    using arm_table_t     = std::map<charm::as::id::arm,     std::function<void(const arm_code_t&)>>;
    using thumb_table_t   = std::map<charm::as::id::thumb,   std::function<void(const thumb_code_t&)>>;
    using jazelle_table_t = std::map<charm::as::id::jazelle, std::function<void(const u8)>>;
