...
u32 generators::ls_misc_instruction(const id::arm id, const arguments &args) {
    ...
    switch (id) {
        case id::arm::STRH: shared::util::swap_bits(binary, 4, 7, 0b1011); break;
        case id::arm::STRD: shared::util::swap_bits(binary, 4, 7, 0b1111); break;
        case id::arm::LDRD: shared::util::swap_bits(binary, 4, 7, 0b1101); break;
        case id::arm::LDRH: 
            shared::util::swap_bits(binary, 4, 7, 0b1011); 
            shared::util::modify_bit(binary, 20, true);
            break;
        case id::arm::LDRSB: 
            shared::util::swap_bits(binary, 4, 7, 0b1101); 
            shared::util::modify_bit(binary, 20, true);
            break;
        case id::arm::LDRSH: 
            shared::util::swap_bits(binary, 4, 7, 0b1111); 
            shared::util::modify_bit(binary, 20, true);
            break;
        default: shared::out::dev_error("Invalid configuration to load/store misc instruction pattern generation");
    }

    // 4th and 7th bits are always 1, so they are ignored

    auto change_immed = [&](const u8 offset_8) -> void {
        const u8 immed_H = shared::util::bit_range<u8>(args.first_int, 4, 7);
        const u8 immed_L = shared::util::bit_range<u8>(args.first_int, 0, 3);
        
        shared::util::swap_bits(binary, 8, 11, immed_H);
        shared::util::swap_bits(binary, 0, 3, immed_L);
    };
    ...
    switch (args.shifter) {
        ...
        default: shared::out::dev_error("Invalid shifter configuration to load/store misc instruction pattern generation");
    }
