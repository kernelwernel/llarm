...
std::string shifters::shifter_to_string(const mode mode, const u32 code, const settings settings) {
    switch (mode) {
        case shifters::mode::DATA_IMM: return data_imm(code, settings);
        case shifters::mode::DATA_IMM_LSL: return data_imm_pattern(code, "LSL", settings);
        case shifters::mode::DATA_IMM_LSR: return data_imm_pattern(code, "LSR", settings);
        case shifters::mode::DATA_IMM_ASR: return data_imm_pattern(code, "ASR", settings);
        case shifters::mode::DATA_IMM_ROR: return data_imm_pattern(code, "ROR", settings);
        case shifters::mode::DATA_REG: return data_reg(code, settings);
        case shifters::mode::DATA_REG_LSL: return data_reg_pattern(code, "LSL", settings);
        case shifters::mode::DATA_REG_LSR: return data_reg_pattern(code, "LSR", settings);
        case shifters::mode::DATA_REG_ASR: return data_reg_pattern(code, "ASR", settings);
        case shifters::mode::DATA_REG_ROR: return data_reg_pattern(code, "ROR", settings);
        case shifters::mode::DATA_RRX: return data_rrx(code, settings);
        case shifters::mode::LS_IMM: return ls_imm(code, settings);
        case shifters::mode::LS_IMM_PRE: return ls_imm_pre(code, settings);
        case shifters::mode::LS_IMM_POST: return ls_imm_post(code, settings);
        case shifters::mode::LS_REG: return ls_reg(code, settings);
        case shifters::mode::LS_REG_PRE: return ls_reg_pre(code, settings);
        case shifters::mode::LS_REG_POST: return ls_reg_post(code, settings);
        case shifters::mode::LS_REG_SCALED_LSL: return ls_reg_scaled(code, "LSL", settings);
        case shifters::mode::LS_REG_SCALED_LSR: return ls_reg_scaled(code, "LSR", settings);
        case shifters::mode::LS_REG_SCALED_ASR: return ls_reg_scaled(code, "ASR", settings);
        case shifters::mode::LS_REG_SCALED_ROR: return ls_reg_scaled(code, "ROR", settings);
        case shifters::mode::LS_REG_SCALED_RRX: return ls_reg_scaled_rrx(code, settings);
        case shifters::mode::LS_REG_SCALED_PRE_LSL: return ls_reg_scaled_pre(code, "LSL", settings);
        case shifters::mode::LS_REG_SCALED_PRE_LSR: return ls_reg_scaled_pre(code, "LSR", settings);
        case shifters::mode::LS_REG_SCALED_PRE_ASR: return ls_reg_scaled_pre(code, "ASR", settings);
        case shifters::mode::LS_REG_SCALED_PRE_ROR: return ls_reg_scaled_pre(code, "ROR", settings);
        case shifters::mode::LS_REG_SCALED_PRE_RRX: return ls_reg_scaled_pre_rrx(code, settings);
        case shifters::mode::LS_REG_SCALED_POST_LSL: return ls_reg_scaled_post(code, "LSL", settings);
        case shifters::mode::LS_REG_SCALED_POST_LSR: return ls_reg_scaled_post(code, "LSR", settings);
        case shifters::mode::LS_REG_SCALED_POST_ASR: return ls_reg_scaled_post(code, "ASR", settings);
        case shifters::mode::LS_REG_SCALED_POST_ROR: return ls_reg_scaled_post(code, "ROR", settings);
        case shifters::mode::LS_REG_SCALED_POST_RRX: return ls_reg_scaled_post_rrx(code, settings);
        case shifters::mode::LS_MISC_IMM: return ls_misc_imm(code, settings);
        case shifters::mode::LS_MISC_IMM_PRE: return ls_misc_imm_pre(code, settings);
        case shifters::mode::LS_MISC_IMM_POST: return ls_misc_imm_post(code, settings);
        case shifters::mode::LS_MISC_REG: return ls_misc_reg(code, settings);
        case shifters::mode::LS_MISC_REG_PRE: return ls_misc_reg_pre(code, settings);
        case shifters::mode::LS_MISC_REG_POST: return ls_misc_reg_post(code, settings);
        case shifters::mode::LS_MUL_INC_AFTER: return ls_mul_inc_after(settings);
        case shifters::mode::LS_MUL_INC_BEFORE: return ls_mul_inc_before(settings);
        case shifters::mode::LS_MUL_DEC_AFTER: return ls_mul_dec_after(settings);
        case shifters::mode::LS_MUL_DEC_BEFORE: return ls_mul_dec_before(settings);
        case shifters::mode::LS_COPROC_IMM: return ls_coproc_imm(code, settings);
        case shifters::mode::LS_COPROC_IMM_PRE: return ls_coproc_imm_pre(code, settings);
        case shifters::mode::LS_COPROC_IMM_POST: return ls_coproc_imm_post(code, settings);
        case shifters::mode::LS_COPROC_UNINDEXED: return ls_coproc_unindexed(code, settings);
