 ...
 * reference: A4-10
 */ 
std::string generators::arm::branching::B(const u32 code, const u32 PC) {
    const u32 signed_immed_24 = shared::util::bit_fetcher(code, 0, 23);

    const u32 target_address = ((signed_immed_24 << 2) + PC + 8);

    return util::make_string("B", util::cond(code), " ", target_address);
}


// basically the same as B above
std::string generators::arm::branching::BL(const u32 code, const u32 PC) {
    const u32 signed_immed_24 = shared::util::bit_fetcher(code, 0, 23);

    const u32 target_address = ((signed_immed_24 << 2) + PC + 8);

    return util::make_string("BL", util::cond(code), " ", target_address);
}
 ...
 * reference: A4-16
 */
std::string generators::arm::branching::BLX1(const u32 code, const u32 PC) {
    const u32 signed_immed_24 = shared::util::bit_fetcher(code, 0, 23);
    const u32 target_address = ((signed_immed_24 << 2) + PC + 8);

    return util::make_string("BLX ", util::hex(target_address));
}
 ...
 * reference: A4-18
 */
std::string generators::arm::branching::BLX2(const u32 code) {
    const std::string Rm = util::reg_string(code, 0, 3);
